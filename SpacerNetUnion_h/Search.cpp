...
namespace GOTHIC_ENGINE {
	...
	int SpacerApp::SearchFillVobClass(bool derived, int type, int selectedCount)
	{
		...
		if (searchType == SearchVobType::ReplaceVobTree)
		{
			...
			//MessageBox(0, "«амен: " + A resultFound.GetNum(), 0, 0);
		}
	...
	Common::Map<CString, int> searchVisualUniqList;
	...
	void SpacerApp::FindVobsVisualsUnique(CString path)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (pVob && pVob->visual)
			{
				...
				if (!foundPair.IsNull())
				{
					foundPair.GetValue() += 1;
				}
				else
				{
					searchVisualUniqList.Insert(key, 1);
				}
			}
		}
		...
		outfile << std::left << std::setw(80) << std::setfill(separator) << "VisualName";
		outfile << std::left << std::setw(10) << std::setfill(separator) << "Amount";
		outfile << std::left << std::setw(16) << std::setfill(separator) << "Type";
		//outfile << std::left << std::setw(6) << std::setfill(separator) << result;
		outfile << std::left << std::setw(45) << std::setfill(separator) << "VDF archive of this visual";
		...
		for (uint i = 0; i < arr.GetNum(); i++)
		{
			...
			if (!pair->IsNull())
			{
				...
				outfile << std::left << std::setw(80) << std::setfill(separator) << originVisualName;
				outfile << std::left << std::setw(10) << std::setfill(separator) << pair->GetValue();
				outfile << std::left << std::setw(16) << std::setfill(separator) << volumeName;
				//outfile << std::left << std::setw(6) << std::setfill(separator) << result;
				outfile << std::left << std::setw(45) << std::setfill(separator) << volumeNameVdf;

				outfile << "\n";

				//outfile << "Name: '" << visualName << "'\tCount: " << pair->GetValue() << "\tVdfName: " << volumeName << " Result: " << result << "\n";
		...
		/*
		for (int i = 0; i < searchVisualUniqList.GetNumInList(); i++)
		{
			auto entry = searchVisualUniqList.Get(i);

			if (entry)
			{
				outfile << "Name: '" << entry->visualName << "' Count: " << entry->count << endl;
			}
		}
		*/
